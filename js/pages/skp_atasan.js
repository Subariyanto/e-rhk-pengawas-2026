// SKP Atasan Langsung — import Excel/Word/PDF, render dinamis sesuai isi file
(function () {
  const STORE_KEY = 'skp_atasan_doc';
  const PDF_WORKER_SRC = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';

  function load() { return Store.get(STORE_KEY, null); }
  function save(doc) { Store.set(STORE_KEY, doc); }
  function clear() { Store.set(STORE_KEY, null); }

  Page.SKPAtasan = function () {
    const doc = load();
    UI.shell('SKP Atasan Langsung', `
      <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
        <input type="file" id="fileSkp" accept=".xlsx,.xls,.docx,.pdf" class="form-control" style="max-width:380px;" />
        ...
        <div class="text-muted small ms-2">Format yang didukung: Excel (.xlsx/.xls), Word (.docx), PDF</div>
      </div>
      ...
      ${doc ? renderDoc(doc) : `
        <div class="card">
          <div class="card-body text-center text-muted py-5">
            ...
            <div class="mt-2"><strong>Belum ada dokumen SKP Atasan.</strong></div>
            <div class="mb-3 small">Import file Excel / Word / PDF untuk menampilkan SKP Atasan Langsung di sini.</div>
          </div>
        </div>
      `}

      <div id="skpRender" class="mt-3"></div>
    `);

    document.getElementById('btnImport').addEventListener('click', async () => {
      const f = document.getElementById('fileSkp').files[0];
    ...
    if (doc) {
      ...
      if (btn) btn.addEventListener('click', async () => {
        if (await UI.confirmDialog('Hapus dokumen SKP Atasan yang sudah diimport?')) {
          clear();
          Page.SKPAtasan();
        }
      });
  ...
  function renderExcel(payload) {
    ...
    const tabs = sheets.map((s, i) => `
      <li class="nav-item">
        <button class="nav-link ${i === 0 ? 'active' : ''}" data-bs-toggle="tab" data-bs-target="#sheet-${i}" type="button">${U.escapeHtml(s.name)}</button>
      </li>`).join('');
    const panes = sheets.map((s, i) => `
      <div class="tab-pane fade ${i === 0 ? 'show active' : ''}" id="sheet-${i}">
  ...
  function renderSheetTable(sheet) {
    ...
    // build a skip-mask so merged child cells are not rendered
    const skip = new Set();
    const mergeMap = new Map(); // "r,c" → {rowspan, colspan}
  ...
  async function parseExcel(file) {
    ...
    wb.eachSheet((ws) => {
      const rows = [];
      const merges = [];
      // collect merges (ws.model.merges is "A1:B2" style)
